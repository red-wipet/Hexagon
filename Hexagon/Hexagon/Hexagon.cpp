...
DWORD WINAPI payload2(LPVOID lpParam) {
	...
	int radius = 40.999f; double angle = 0;
	for (;;) {
		desk = GetDC(0);
...
int main() {
	if (MessageBoxW(NULL, L"Run GDI only?", L"Hexagon brights in the sky upon the death...", MB_YESNO | MB_ICONWARNING) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure?! EPILEPSEY WARNING!!!", L"is it too early to destroy pc???", MB_YESNO | MB_ICONWARNING) == IDNO)
		{
			ExitProcess(0);
		}
		else
